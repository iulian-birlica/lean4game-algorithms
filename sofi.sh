...
do
    ...
    sed_inplace "s/^Level.*$/Level $((n+1))/" "$FILE"
    ...
    do
        ...
        sed_inplace "s/^import Game\.Levels\.${world_name}\.${old_names[$k]}[[:space:]]*$/import Game.Levels.${world_name}.${new_names[$k]}/" "$FILE"
    done
    ...
    do
        ...
        sed_inplace "s/^import Game.Levels.${world_name}.${old_names[$k]}[[:space:]]*$//" "$FILE"
    done
    ...
    if (( n == 0 )); then
        sed_inplace "/^import Game\.Levels\.${world_name}.*$/d" "$FILE"
        # Test this sed syntax with:
        # echo -e "import Game.Levels.abc\nQVM\nimport Game.Levels.qvm" | sed "/^import Game\.Levels.*$/d"
    else
        sed_inplace "0,/^import Game\.Levels\.${world_name}.*$/s/^import Game\.Levels\.${world_name}.*$/import Game.Levels.${world_name}.${new_names[$((n-1))]}/" "$FILE"
        # Test with sed syntax with:
        # echo -e "import Game.Levels.abc\nQVM\nimport Game.Levels.qvm" | sed "0,/^import Game\.Levels.*$/s/^import Game\.Levels.*$/Super/"
    fi
done
...
temp_file="$(mktemp "${TMPDIR:-/tmp}/sofi.XXXXXX")"
printf "%b" "$new_imports" > "$temp_file"
cat "$FILE" >> "$temp_file"
mv "$temp_file" "$FILE"
